...
import { okInput } from "@/components/common/Constants";
...
export default function AppConfigPage() {
  ...
  const companyByKey = useMemo(
    () =>
      Object.fromEntries(
        items.filter((r) => r.section === "company").map((r) => [r.key, r])
      ),
    [items]
  );
  ...
  return (
    <div className="flex flex-col h-full min-h-0 p-2 md:p-3">
      <form
        onSubmit={handleSubmit}
        className="bg-white border border-slate-300 rounded-sm shadow-sm flex flex-col flex-1 min-h-0 overflow-hidden"
      >
        {/* Form header */}
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-5">
          {loading ? (
            ...
            <div className="w-full space-y-8">
              ...
              {/* Application — DB config after go-live (not server .env) */}
              <section>
                <div className="mb-4 pb-2 border-b border-slate-200">
        ...
        {!loading && (
          <div className="shrink-0 px-4 py-3 border-t border-slate-200 bg-slate-50/80 flex flex-col sm:flex-row sm:items-center sm:justify-end gap-2">
            <button type="button" disabled={saving || dirtyRows.length === 0} onClick={handleReset} className="px-5 py-2.5 text-sm font-bold text-slate-500 hover:text-slate-700 disabled:opacity-40">
              Reset
            </button>
            <button type="submit" disabled={saving || dirtyRows.length === 0}
