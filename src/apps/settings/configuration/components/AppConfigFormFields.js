...
export function ConfigFormRow({ row, draft, setDraft, disabled }) {
  ...
  const isSelect = row.value_type === "select" && Array.isArray(row.options) && row.options.length > 0;
  const isOn = String(value).toLowerCase() === "true" || value === "1";
  const label = row.key === "company_address" ? "Address" : row.label;
  const help = row.description ? String(row.description).trim() : "";
  ...
  if (isSelect) {
    const allowed = row.options.map((o) => String(o.value));
    const selectValue = allowed.includes(String(value)) ? String(value) : String(row.options[0].value);
    return (
      <div className="space-y-1 min-w-0">
        <label htmlFor={`cfg-${row.key}`} className={CONFIG_LABEL}>
          {label}
