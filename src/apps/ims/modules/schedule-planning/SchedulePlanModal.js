...
import { Loader2, History, PackageMinus, Calendar, Check } from "lucide-react";
...
import { IMS_DRAWER_FOOTER_WRAP, IMS_DRAWER_BTN_CANCEL, IMS_DRAWER_BTN_PRIMARY } from "../../lib/helpers/masterListUi";
...
function ScheduleTargetDateField({ value, onChange, className = "", placeholder = "DD/MM/YYYY", minDate, maxDate, onOutOfRange, hasError = false, title }) {
  ...
  const commitDisplay = useCallback(
    (raw) => {
      ...
      if (!ymd) {
        setText(filterDateToDisplay(value));
        return;
      }
      if (!inRange(ymd)) {
        rejectOutOfRange();
    ...
    [commitYmd, inRange, onChange, rejectOutOfRange, value]
  );
...
export default function SchedulePlanModal({
  ...
  const drawerFooter = (
    <div className={IMS_DRAWER_FOOTER_WRAP}>
      <button type="button" onClick={handleClose} disabled={saving} className={IMS_DRAWER_BTN_CANCEL}>
        Cancel
      </button>
      <button
        type="button"
