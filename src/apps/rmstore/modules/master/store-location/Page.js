...
export default function LocationMasterPage() {
  ...
  const fetchLocations = useCallback(async () => {
    ...
    try {
      ...
      toast.error(err?.message || "Could not load the store locations. Please try again.");
      setAllRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const { openNewModal, openEditModal, openPrintModal, openDeleteModal, tableHotkeyProps } = useListDrawerHotkeys({
    ...
    printBlockedMessage: "Select an authorized location to print its QR label (Ctrl+Alt+P).",
    printModule: "rm_store_location_master",
    printAction: "view",
  ...
  const HEADERS = [
    ...
    ["RM Rack", "rack_no", (v) => <span className="font-bold text-slate-800 uppercase text-[11px]">{v || "—"}</span>, { width: "90px" }],
    ["RM Row", "row_no", (v) => <span className="font-bold text-slate-800 uppercase text-[11px]">{(v || "—").toString().toUpperCase()}</span>, { width: "90px" }],
    ["Item Code", "item_code", (v, row) => (
      <div className="flex flex-col leading-tight min-w-0">
        <span className="font-mono text-[10px] font-bold tracking-tighter truncate" title={v || ""}>{v || "—"}</span>
      </div>
    ), { width: "160px" }],
    ["Item Description", "item_desc", (v) => (
      <div className="flex flex-col leading-tight min-w-0">
        <span className="text-[10px] truncate" title={v || ""}>{v || "—"}</span>
      </div>
    ), { width: "160px" }],
  ...
  const { exporting, handleExport, exportDisabled } = useListPageExport({
    moduleName: "RM Store Location Master",
    rows: filteredRows,
    headers: HEADERS,
  });
  ...
  return (
    <div className={LIST_PAGE_SHELL}>
        ...
        <ListPageToolbar>
          <ListPageToolbarLayout
            actions={
              ...
              <PrintActionButton module="rm_store_location_master" variant="outline" label="Print QR" icon={Printer} disabled={!selected || !selectedRecord?.approved} onClick={openPrintModal} title="Print QR label (Ctrl+Alt+P / Ctrl+P in app)" className="rounded-none h-9 bg-white text-[11px] font-bold uppercase px-4 border-slate-300 shadow-none" />
      ...
      {deleteItem && (
        <DeleteModal item={deleteItem} onClose={() => setDeleteItem(null)} onSuccess={() => { fetchLocations(); setSelected(null); }} service={locationService} entityLabel="Location" idKey="location_id" moduleSlug="rm_store_location_master" />
      )}
