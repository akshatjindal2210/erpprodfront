...
export default function AuditModal({ open, onClose, onSuccess, editData, mode = "add" }) {
  ...
  const canApprove = canAccess("rm_inventory_audit", "authorize").allowed;
  ...
  const handleRandomAssign = async () => {
    ...
    try {
      const res = await storeLocationService.getViews({ page: 1, limit: 5000, permission_module: "rm_inventory_audit", permission_action: "view" });
      const list = Array.isArray(res?.data) ? res.data : Array.isArray(res) ? res : [];
  ...
  const handleApprove = async () => {
    ...
    try {
      await inventoryAuditService.update(editData.audit_id, { approved: true });
  ...
  const handleSave = async (statusOverride = null, actionKey = "save") => {
    ...
    try {
      ...
      const request = isEdit
        ? inventoryAuditService.update(editData.audit_id, payload)
        : inventoryAuditService.create(payload);
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <SearchableSelect
                    ...
                    getByIdService={(id) => storeLocationService.getById(id)}
                    dataKey="location_id"
