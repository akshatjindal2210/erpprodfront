...
export default function RmInventoryAuditPage() {
  const canAccess = useCanAccess();
  const viewAccess = useMemo(() => canAccess("rm_inventory_audit", "view"), [canAccess]);
  const addAccess = useMemo(() => canAccess("rm_inventory_audit", "add"), [canAccess]);
  const authorizeAccess = useMemo(() => canAccess("rm_inventory_audit", "authorize"), [canAccess]);
  const editAccess = useMemo(() => canAccess("rm_inventory_audit", "edit"), [canAccess]);
  const deleteAccess = useMemo(() => canAccess("rm_inventory_audit", "delete"), [canAccess]);
  ...
  const loadExecutionAudit = useCallback(async (auditId) => {
    ...
    try {
      const res = await inventoryAuditService.getById(auditId);
  ...
  const openAuditExecution = useCallback(
    async (locationRow) => {
      ...
      try {
        // Freeze expected coils for this assigned location at the moment user starts
        await inventoryAuditService.startLocation({
          audit_id: locationRow.audit_id,
          location_id: locationRow.location_id,
        });
      } catch (err) {
        toast.error(err?.message || "Failed to lock expected coils for this location");
  ...
  const fetchAudits = useCallback(async () => {
    ...
    try {
      const { data } = await fetchAllListPages(async (page, limit) => {
        const body = await inventoryAuditService.getAll({
          page,
          limit,
  ...
  const handleReopenLocation = async () => {
    ...
    setReopening(true);
    try {
      const res = await inventoryAuditService.reopenLocation({
  ...
  return (
    <div className={IMS_LIST_PAGE_SHELL}>
      ...
      {deleteItem && (
        <DeleteModal item={deleteItem} onClose={() => setDeleteItem(null)} onSuccess={() => { fetchAudits(); setSelected(null); }} service={inventoryAuditService} entityLabel="Audit" idKey="audit_id" moduleSlug="rm_inventory_audit" />
      )}
    </div>
  );
}
