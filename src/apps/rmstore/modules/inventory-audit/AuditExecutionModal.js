...
export default function AuditExecutionModal({ open, onClose, onSuccess, auditData, fixedLocationId }) {
  ...
  const handleScanValue = async (val) => {
    ...
    if (looksLikeLocation) {
      showScanToast("error", "audit-location-already", "Location already verified. Scan coils now.", 2000);
      setScannedInput("");
      return;
    }
    ...
    const boxCode = extractCoilUid(val);
    if (!boxCode) {
      showScanToast("error", "audit-invalid-coil", "Scan the coil sticker QR code.", 2200);
      setScannedInput("");
      return;
    }
    ...
    setLoading(true);
    try {
      const coil = await coilService.getViews({
        coil_no_uid: boxCode,
        permission_module: "rm_inventory_audit",
        permission_action: "add",
      });
      if (coil?.data) {
        const res = await inventoryAuditService.submitScan({
          audit_id: auditData.audit_id,
          location_id: locId,
          coil_no_uids: [boxCode],
          complete_location: false,
        });

        setScannedData((prev) => {
          const coils = prev[locId] || [];
          if (coils.includes(boxCode)) return prev;
          return { ...prev, [locId]: [boxCode, ...coils] };
        });
        ...
        if (res?.data?.comparison?.exact) {
          showScanSuccess(
            "audit-all-scanned",
            "All coils scanned — tap Submit Location when ready",
            3500
          );
        } else {
          showScanSuccess(`audit-coil-${boxCode.toLowerCase()}`, SCAN_SNACK_MSG.BOX_ADDED(boxCode), 1200);
        }
      } else {
        showScanToast("error", `audit-coil-missing-${boxCode.toLowerCase()}`, `coil ${boxCode} was not found in the system.`, 2200);
      }
    } catch {
      showScanToast("error", "audit-coil-save-failed", "Failed to save the coil scan.", 2200);
    } finally {
  ...
  const laserPreviewLabel = useCallback(
    (raw) => (isLocationScanned ? coilUidDisplayLabel(raw) : locationNoDisplayLabel(raw)),
    [isLocationScanned]
  );
  ...
  const removeBox = async (uid) => {
    ...
    try {
      await inventoryAuditService.removeScan({
        audit_id: auditData.audit_id,
        location_id: locId,
        coil_no_uid: uid,
      });
      ...
      toast.success(`coil ${uid} removed`);
    } catch {
      toast.error("Failed to remove coil scan");
    }
  };
  ...
  const handleCompleteLocation = async () => {
    ...
    try {
      const res = await inventoryAuditService.submitScan({
        audit_id: auditData.audit_id,
        location_id: locId,
        coil_no_uids: scannedData[locId] || [],
      ...
      <div className="space-y-2.5 pb-3">
        ...
        <QrScannerOverlay
          ...
          hint={
            !isLocationScanned
              ? "Scan location"
              : `coils · ${currentScannedBoxes.length} scanned`
          }
        ...
        <div className="rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-2 space-y-1">
          ...
          <p
            ...
            role="status"
          >
            {isLocationScanned ? "Scan each coil" : "First scan location, then coils"}
          </p>
        </div>
        ...
        <div className="space-y-2">
            ...
            <div className="flex items-stretch gap-2 w-full min-w-0">
              {showPhoneQr && (
                <button
                  ...
                  title={isLocationScanned ? "Scan coil QR" : "Scan location QR"}
                >
                  <QrCode size={16} />
              ...
              {laserScan && (
                <LaserScanField
                  ...
                  armButtonLabel={isLocationScanned ? "Scan coil" : "Scan Loc"}
                  fill={scanBtnCount > 0}
                />
              )}
            </div>
          ...
          <form onSubmit={handleScanSubmit} className="relative w-full min-w-0">
            ...
            <ScanEnterInput
              ...
              placeholder={
                isLocationScanned
                  ? "Type coil code, then press Enter"
          ...
          <div className="space-y-1.5">
            ...
            <div className="max-h-[200px] overflow-y-auto space-y-1 pr-0.5 custom-scrollbar">
              {currentScannedBoxes.length === 0 ? (
                <p className="py-4 text-center text-[10px] text-slate-400 italic">No coils yet</p>
              ) : (
