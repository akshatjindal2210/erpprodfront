...
export function buildAuditLocationHeaders({ canViewAudit, openLocationComparison, scannerOnly = false }) {
  ...
  return [
    ...
    ["coils", "expected_count", (v, row) => renderLocationBoxesCell(row), {
      width: "130px",
      wrap: true,
    ...
    ["Difference", "difference_boxes", (v, row) => {
      ...
      const coils = Array.isArray(v) && v.length ? v : [...missing, ...extra];
      const inner = !coils.length ? (
      ...
      copyValue: (item) => {
        ...
        const coils = [...missing, ...extra];
        return coils.length ? coils.join(", ") : "All matched";
      },
    }],
    ["Date Range", "start_date", (v, row) => (
      <span className="text-[10px] font-bold text-slate-700">{formatDate(row.start_date)} — {formatDate(row.end_date)}</span>
