      ...
      <table className="w-full min-w-[980px] text-left border-collapse">
        <thead className="sticky top-0 z-[1] bg-slate-50 border-b border-slate-200">
          <tr>
            ...
            <th className="px-3 py-2 text-[10px] font-black uppercase text-slate-500">MRN no.</th>
            <th className="px-3 py-2 text-[10px] font-black uppercase text-slate-500">Coil UID</th>
            ...
            <th className="px-3 py-2 text-[10px] font-black uppercase text-slate-500">Coil location</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row, idx) => (
            <tr
              key={`${row.coil_no_uid}-${row.difference_type}-${idx}`}
            ...
            >
              ...
              <td className="px-3 py-2 text-[10px] font-medium text-slate-700">{row.mrn_no ?? "—"}</td>
              <td className="px-3 py-2 text-[10px] font-mono font-bold text-slate-800">{row.coil_no_uid}</td>
  ...
  return (
    <details className="rounded-xl border border-emerald-200 bg-emerald-50/30">
      <summary className="px-3 py-2.5 text-[10px] font-black uppercase text-emerald-800 cursor-pointer select-none flex items-center gap-2">
        ...
        Matched coils ({totalMatched}) — expected &amp; scanned align
      </summary>
      <div className="p-2 border-t border-emerald-100 bg-white/80">
        <BoxDetailTable
          rows={rows}
          showLocation={showLocation}
          emptyMessage="No matched coils"
          variant="matched"
        />
      </div>
    </details>
...
function locScorePct(loc) {
  return computeLocationScoreFromCounts(
    ...
    loc.extra_scan_count ?? loc.extra_coils?.length ?? 0
  );
}
...
export default function AuditComparisonModal({
  ...
  const loadReport = useCallback(async () => {
    ...
    const loadFromApi = async () => {
      ...
      const res = await inventoryAuditService.getComparisonReport(id, locationId);
      if (!res?.success) {
  ...
  const handleCompleteLocation = async () => {
    ...
    try {
      const res = await inventoryAuditService.completeLocation({
  ...
  const handleAdjustment = async () => {
    ...
    setActionLoading(true);
    try {
      const res = await inventoryAuditService.applyComparisonAdjustment({
            ...
            <div className="flex flex-wrap items-start gap-2">
              <div className="flex flex-wrap items-center gap-x-2 gap-y-1.5 min-w-0 flex-1">
                ...
                <p className={`text-[11px] font-bold shrink-0 ${allMatched ? "text-emerald-800" : "text-amber-800"}`}>
                  {allMatched ? "All coils match" : `${stats.notScanned} missing · ${stats.extra} extra`}
                </p>
                  ...
                  {locations.map((loc) => (
                    <tr key={loc.location_id} className="border-b border-slate-100">
                      ...
                      <td className="px-2 py-1.5 text-amber-700 font-bold">
                        {loc.not_scanned_count ?? loc.missing_coils?.length ?? 0}
                      </td>
