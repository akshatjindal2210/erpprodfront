function parseJsonCoilList(raw) {
  if (!raw) return [];
...
/** Parse expected_coils JSON array on a location row. */
export function parseExpectedCoils(loc) {
  const raw =
    loc != null && typeof loc === "object" && !Array.isArray(loc) && !("expected_coils" in loc)
      ? parseJsonCoilList(loc)
      : parseJsonCoilList(loc?.expected_coils);
  return raw.map(normalizeExpectedCoil).filter((row) => row?.coil_no_uid);
}

function normalizeExpectedCoil(row) {
  if (!row || typeof row !== "object") return null;
  const coil_no_uid = String(row.coil_no_uid || "").trim().toUpperCase();
  if (!coil_no_uid) return null;
  const acc_name = resolveCoilAccName(row);
  return {
    ...row,
    coil_no_uid,
    ...(acc_name ? { acc_name } : {}),
...
/** Parse scanned_coils JSON array on a location row. */
export function parseScannedCoils(loc) {
  if (loc != null && typeof loc === "object" && !Array.isArray(loc) && !("scanned_coils" in loc)) {
    return parseJsonCoilList(loc);
  }
  const list = parseJsonCoilList(loc?.scanned_coils);
  return list
    .map((row) => ({
...
export function parseAssignmentHistory(loc) {
  ...
  return list
    .map((entry) => {
      const coils = Array.isArray(entry?.scanned_coils) ? entry.scanned_coils : [];
      return {
        ...
        expected_coils: Array.isArray(entry?.expected_coils) ? entry.expected_coils : [],
        scanned_coils: coils,
        scan_count: Number(entry?.scan_count) || coils.length,
      };
    })
    .filter((entry) => entry.user_id != null);
}

/** Clone history + active scans merged (unique by coil uid). */
export function getMergedScannedBoxes(loc) {
  const byUid = new Map();

  for (const entry of parseAssignmentHistory(loc)) {
    for (const row of entry.scanned_coils || []) {
      const uid = String(row?.coil_no_uid || "").trim().toUpperCase();
      if (uid) byUid.set(uid, row);
    }
  }
  for (const row of parseScannedCoils(loc)) {
    const uid = String(row?.coil_no_uid || "").trim().toUpperCase();
    if (uid) byUid.set(uid, row);
  }
...
function buildLocationListRow(audit, loc) {
  ...
  const scans = parseScannedCoils(loc);
  ...
  return {
    ...
    expected_coils: loc.expected_coils,
    scanned_coils: loc.scanned_coils,
...
export function getLocationUserScanSummary(loc) {
  ...
  const currentScans = parseScannedCoils(loc);
...
export function countExpectedBoxes(loc) {
  return parseExpectedCoils(loc).length;
}

export function countScannedBoxes(loc) {
  return parseScannedCoils(loc).length;
}
...
/** Build { [location_id]: coil_no_uid[] } from location scanned_coils JSON. */
export function buildScannedDataFromAudit(audit) {
  ...
  for (const loc of audit?.locations || []) {
    ...
    data[locId] = parseScannedCoils(loc)
      .map((row) => String(row?.coil_no_uid || "").trim())
      .filter(Boolean);
  }
...
export function getLocationBoxComparison(loc) {
  const expected = new Set(
    parseExpectedCoils(loc).map((b) => normalizeUid(b.coil_no_uid)).filter(Boolean)
  );
  const scanned = new Set(
    parseScannedCoils(loc).map((s) => normalizeUid(s.coil_no_uid)).filter(Boolean)
  );
...
function formatBoxCustomer(detail) {
  if (!detail) return "—";
  return detail.acc_name || resolveCoilAccName(detail) || "—";
}
...
function buildDifferenceRowsFromComparison(row, cmp) {
  const expected = parseExpectedCoils(row);
  const byUid = new Map(
    expected.map((b) => [normalizeUid(b.coil_no_uid), b]).filter(([uid]) => uid)
  );
...
export function buildLocationComparisonReport(row) {
  ...
  const system_boxes = parseExpectedCoils(row)
    .map((b) => normalizeUid(b.coil_no_uid))
    .filter(Boolean)
    .sort();
  const scanned_coils = parseScannedCoils(row)
    .map((s) => normalizeUid(s.coil_no_uid))
    .filter(Boolean)
    .sort();
