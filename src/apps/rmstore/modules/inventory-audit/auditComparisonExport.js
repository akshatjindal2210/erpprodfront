...
function buildBoxColumns(showLocation) {
  ...
  cols.push(
    ...
    { label: "MRN no.", getValue: (r) => r.mrn_no ?? "" },
    { label: "Coil UID", getValue: (r) => r.coil_no_uid ?? "" },
...
function exportComparisonCsv(ctx, filename) {
  ...
  lines.push(...sectionToCsv("MISSING coils", notScannedRows, boxCols));
  lines.push(...sectionToCsv("EXTRA coils", extraRows, boxCols));
  lines.push(...sectionToCsv("MATCHED coils", matchedRows, boxCols));
...
<style>
  ...
  ${metaHtml}
  ${locHtml}
  ${tableHtml("Missing coils", notScannedRows, boxCols)}
  ${tableHtml("Extra coils", extraRows, boxCols)}
  ${tableHtml("Matched coils", matchedRows, boxCols)}
</body></html>`;
