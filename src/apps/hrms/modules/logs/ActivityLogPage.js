...
import {
  ...
  formatActivityLogActionLabel,
  getActivityLogActionBadgeClass,
...
const MODULE = "hrms_activity_logs";
...
export default function ActivityLogPage() {
  ...
  const HEADERS = [
    [
      ...
      { fixed: true, width: "50px", align: "center" },
    ],
    [
      "Action",
      "action_type",
    ...
    [
      "Module / Entity",
      "module",
      (_v, row) => <ActivityLogModuleEntityCell row={row} appType="hrms" />,
      { width: "180px" },
    ],
    [
      "Details",
      ...
      (v, row) => {
        ...
        const renderSection = (section) => (
          <div key={section.title} className="flex flex-col gap-1">
            <span className="text-[8px] font-black uppercase tracking-widest text-indigo-500">{section.title}</span>
            <div className="flex flex-wrap gap-1">
              {Object.entries(section.data || {}).map(([key, value]) => (
                ...
                >
                  <span className="text-[7px] text-slate-400 uppercase font-black leading-none mb-0.5">{key}</span>
