import { ROW_META, EMPTY_META, TASK_COLORS, DONE_STATUSES, ACTIVE_STATUSES } from "@/apps/task/lib/ui/tasks_common_component/TaskConstant";
...
function resolveAlertMeta(task, currentUserId) {
  ...
  if (task.has_unseen_updates && !DONE_STATUSES.includes(task.status)) return ROW_META.updated;
...
function getActionReasons(task, currentUserId) {
  ...
  if (task.has_unseen_updates && !DONE_STATUSES.includes(task.status))                                                                                           reasons.push("updated");
