...
import { applyClientSearch, defaultSearchParts, sortRowsByKey, nextSortParams } from "@/ui/common/list/clientListSearch";

export const MASTER_LIST_PAGE_SIZE = 100;
...
export function useMasterClientList({
  loadData,
  errorMessage = "Failed to load data",
  getSearchParts,
  preFilter,
  pageSize = MASTER_LIST_PAGE_SIZE,
  getRowKey,
  ...
  const filteredData = useMemo(() => {
    let data = preFilter ? preFilter(allData) : allData;
    const q = tempSearch.trim();

    if (q) {
      const getParts = (row) => {
        const custom = typeof getSearchParts === "function" ? getSearchParts(row) : [];
        const out = Array.isArray(custom) ? [...custom] : [];
        out.push(...defaultSearchParts(row));
        return out;
      };
      data = applyClientSearch(data, q, { getParts, skipSort: !!params.sortKey });
    }

    return sortRowsByKey(data, params.sortKey, params.sortDir);
  }, [allData, tempSearch, params.sortKey, params.sortDir, preFilter, getSearchParts]);
  ...
  const refreshAndKeepSelection = useCallback(
    async (selectionKey) => {
      ...
      if (!selectionKey || !getRowKey) {
        setSelected(null);
        return list;
      }
      const nextIndex = indexRowsByKey(list, getRowKey);
      setSelected(nextIndex.has(selectionKey) ? selectionKey : null);
      return list;
    },
    [reload, getRowKey]
  );
