...
import { ENDPOINTS } from "@/apps/production/lib/config/endpoints";
...
export const masterService = {
  getItems: async (params = {}) => {
    const res = await api(ENDPOINTS.MASTER.ITEMS.LIST, {
      method: "POST",
      body: {
        permission_module: "production_master",
  ...
  getItemsViews: async (params = {}) => {
    const { permission_module = "production_shortage", permission_action = "view", ...rest } = params;
  ...
  getItemViewById: (id, perms = {}) =>
    api(ENDPOINTS.MASTER.ITEMS.VIEWS, {
      ...
      body: {
        id,
        permission_module: perms.permission_module ?? "production_shortage",
