...
// Components
import StockAdjustmentModal from "@/components/stock-adjustment/StockAdjustmentModal";
import StockAdjustmentStickerCloneDrawer from "@/components/stock-adjustment/StockAdjustmentStickerCloneDrawer";
import { plainRemarksForDisplay } from "@/components/stock-adjustment/StockAdjustmentModal";
...
import { printAllStickersForStockAdjustmentAdd } from "@/components/stock-adjustment/stockAdjustmentStickerPrint";
...
export default function StockAdjustmentPage() {
  ...
  const handlePrintStickers = useCallback(async () => {
    ...
    setPrinting(true);
    try {
      const printRes = await printAllStickersForStockAdjustmentAdd(selectedRecord);
      if (!printRes?.ok) {
        if (printRes?.reason === "popup_blocked") {
          toast.warning("Allow pop-ups to print stickers.");
        } else if (printRes?.reason === "no_boxes") {
          toast.error("No boxes found for this adjustment.");
        } else {
          toast.error("Could not print stickers.");
        }
      } else {
        toast.success("Sticker print opened.");
      }
    } catch (err) {
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <ActionButton module="stock_adjustment" action="view" variant="outline" label="View" icon={Eye} disabled={!selected} record={selectedRecord}
                ...
                className="rounded-none h-9 bg-white text-[11px] font-bold uppercase px-4 border-slate-300 text-slate-700 shadow-none"
              />
              ...
              />
              <ActionButton module="stock_adjustment" action="authorize" variant="outline" label={printing ? "Printing…" : "Print stickers"} icon={Printer}
                disabled={
                  !selected ||
                  selectedRecord?.entry_type !== "add" ||
                ...
                title="Print all stickers for selected adjustment (Ctrl+Alt+P / Ctrl+P in app)"
              ...
              <button onClick={() => fetchData()} className="h-9 px-3 border border-slate-300 bg-white text-slate-600 hover:bg-slate-50 rounded-none flex items-center justify-center shadow-none transition-all">
