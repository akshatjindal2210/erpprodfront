...
import { Check, AlertCircle, Loader2, ShieldCheck, Hash, Truck, User, Package, ChevronRight, CheckCircle2, QrCode, ScanLine, Camera, X, MapPin, CheckCircle } from "lucide-react";
...
import { SCAN_SNACK_MSG, useScanSnackbarActions } from "@/global";
...
import { createScanBatchQueue } from "@/helpers/scanBatchQueue";
...
import { boxInventoryStatus, isBoxAvailableForOutEntryScan, isStockAdjustmentOut, outEntryBoxStatusLabel } from "@/utils/boxInventory";
import {
...
} from "@/utils/outEntryFulfillment";
          ...
          {isApprove ? (
            <>
              ...
              <button onClick={() => handleSave(true)} disabled={loading || isBlockingDataLoad || pendingScanCount > 0 || !isFulfillmentComplete} title={!isFulfillmentComplete ? OUT_ENTRY_APPROVE_BLOCKED_MSG : undefined} className="min-w-[140px] px-6 py-2 text-sm font-bold text-white bg-emerald-600 rounded-xl shadow-lg shadow-emerald-100 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                {loading ? <Loader2 size={18} className="animate-spin" /> : <ShieldCheck size={18} />} Authorize exit
              </button>
            </>
          ) : (
            <button
