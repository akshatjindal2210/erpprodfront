...
import { getRichTextExtensions, RICH_TEXT_EDITOR_SURFACE_CLASS } from "./richTextTiptap";
...
function ToolbarBtn({ onClick, active, disabled, title, children }) {
  return (
    <button
      ...
      disabled={disabled}
      title={title}
      className={`p-1.5 rounded-md transition-all text-slate-500 hover:text-slate-800 hover:bg-slate-100 disabled:opacity-30 disabled:cursor-not-allowed ${
...
export default function RichTextEditor({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: RICH_TEXT_EDITOR_SURFACE_CLASS,
      },
    },
  });
