...
import { buildLocationLabelDataUrlsForRows, downloadLocationLabelDataUrl, getLocationDisplayNo, printLocationLabelDataUrls } from "@/features/apps/ims/helpers/locationQrLabel";
...
export default function LocationBulkQRDrawer({
  ...
  const runBulk = useCallback(
    async (mode, rowsOverride = null) => {
      ...
      if (rows.length > 500) {
        toast.info(
          `Preparing ${rows.length} labels — this may take a few minutes. Please keep this tab open.`,
          { autoClose: 6000 }
        );
      }
      ...
      try {
        ...
        if (mode === "print") {
          ...
          toast.success(`Printing ${rows.length} label(s) in one job…`);
        } else {
        ...
        toast.error(err?.message || "Bulk QR export failed");
      } finally {
        setBusy(false);
        setProgress(null);
      }
  ...
  return (
    <Drawer
      ...
      description={
        canPrint
          ? "Print or download all authorized locations in one action (500, 1000, or more). No manual 100-at-a-time steps — processing is automatic."
          : "View permission required to print or download location labels."
      }
      maxWidth="max-w-lg"
    >
        ...
        <div className="flex-1 min-h-0 max-h-[min(55vh,28rem)] overflow-y-auto custom-scrollbar border border-slate-200 rounded-lg divide-y divide-slate-100">
          {authorized.length === 0 ? (
            <p className="p-6 text-center text-xs text-slate-500">
              No authorized locations in the current list. Approve locations first.
            </p>
          ) : (
            authorized.map((row) => {
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block text-[10px] text-slate-500 mt-0.5">
                      Rack {row.rack_no || "—"} · Shelf {(row.shelf_no || "—").toString().toUpperCase()}
