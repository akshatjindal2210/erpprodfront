...
function pickScheduleStatusFromItems(items = []) {
  ...
  const codes = items.map((i) => {
    ...
    return Number(i?.is_planned ?? SCHEDULE_PLAN_STATUS.PENDING);
  });
...
function renderScheduleCompareCell(row, field, opts = {}) {
  ...
  return (
    <CompareImsDbLines
      imsText={formatComparePlain(cmp.ims, opts)}
...
function renderScheduleImsParty(_v, row) {
  if (row?.comparison?.missing_ims) return <span className="text-[10px] text-slate-400">—</span>;
