...
import { userService as taskUserApi } from "@/features/apps/task/services/userApi";
...
import { AppConfigFormLoading, CONFIG_INPUT, CONFIG_LABEL, CONFIG_TEXTAREA } from "@/features/admin/configuration/components/AppConfigFormFields";
...
function formatUserLabel(u) {
  return formatTaskUserOptionLabel(u);
}
...
export default function SendMessageTab({ templates = [] }) {
  ...
  const userOptions = useMemo(
    () => users.map((u) => ({ id: u.id, name: formatUserLabel(u) })),
    [users]
  );
