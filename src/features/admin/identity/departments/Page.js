...
import { Plus, Briefcase, RefreshCcw, Edit3, Trash2, X } from "lucide-react";
  ...
  const fetchDepartments = useCallback(async () => {
    ...
    try {
      const { data, total } = await fetchAllListPages(
        async (page, limit) => {
          const body = await departmentService.getAll({
            page,
            limit,
            filters: {
        ...
        <div className="px-3 py-2 bg-white border-b border-slate-200 flex flex-col gap-2 shrink-0">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2 flex-wrap">
              <ActionButton module="departments" action="add" label="New" icon={Plus} onClick={openNewModal}
                className="rounded-none h-9 text-[11px] font-bold uppercase tracking-wider px-4 shrink-0 shadow-none border-slate-300"
              />
              <ActionButton module="departments" action="edit" variant="outline" label="Edit" icon={Edit3} disabled={selected === null} record={selectedRow} onClick={openEditModal}
              ...
              />
              <ActionButton module="departments" action="delete" variant="danger" label="Delete" icon={Trash2} disabled={selected === null} record={selectedRow} onClick={() => setDeleteItem(selectedRow)} 
        ...
        <div className="px-3 py-1.5 bg-slate-50 border-t border-slate-200 flex items-center justify-between shrink-0">
          <span className="text-[11px] text-slate-500">
            {tempSearch.trim()
              ...
              : `${displayRows.length} / ${totalItems} departments`}
          </span>
        </div>
      </div>

      {modalOpen && (
        <DepartmentModal open={modalOpen}
          onClose={() => {
            setModalOpen(false);
            setEditItem(null);
          }}
